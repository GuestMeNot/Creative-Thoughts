...
One annoyance for users when there is lots of data entry is the lack immediate 
feedback for invalid data entry. Adding a server round-trip to the mix 
compounds this annoyance and is a waste of electrons. 
...
as needed to visually display this lifecycle state to the user at a glance.
...
indicate to the user the state of a component:
 ...
 - [Asynchronously](https://www.w3schools.com/js//js_asynchronous.asp) loading of data 
   using a library such as [Axios Http](https://axios-http.com/) puts Front-End 
   Components get out of phase with the underlying data state. During the wait time 
   to receive data from a server, the old data in a component is no longer valid yet 
   the new data is not yet available. A user should be able to continue interacting 
   with the application while data is loading. For example, perhaps the user made a 
   mistake and want to load different data instead. Locking the entire application is 
 ...
 - When saving data, buttons and relevant fields being saved should be styled such
   that they
...
A field should not be always styled with a red border. These styles should be placed
...
Each state for each component then can be ties using BEM to a unique set of styles which are
